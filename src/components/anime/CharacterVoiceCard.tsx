...
export function CharacterVoiceCard({ edge }: CharacterVoiceCardProps) {
    ...
    return (
        <div className="flex bg-white/5 border border-white/10 rounded-xl overflow-hidden hover:bg-white/10 transition-all h-[80px]">
            {/* Character Info */}
            <Link href={`/character/${character.id}`} className="flex flex-1 min-w-0 pr-2 hover:bg-white/5 transition-colors group">
            ...
            {/* Voice Actor Info */}
            {voiceActor && (
                <Link href={`/person/${voiceActor.id}`} className="flex flex-1 min-w-0 flex-row-reverse pl-2 hover:bg-white/5 transition-colors border-l border-white/5 group">
                    <div className="relative w-14 h-full shrink-0">
                        <Image
