...
import { BookOpen, CheckCircle, Clock1, PauseCircle, XCircle, ArrowLeft, BarChart3 } from 'lucide-react'
...
export default function UserMangaListPage() {
    ...
    const statusOptions = [
        { value: 'READING', label: 'Reading', color: 'bg-blue-500', icon: BookOpen },
        ...
        { value: 'PLAN_TO_READ', label: 'Planned', color: 'bg-yellow-500', icon: Clock1 },
    ]
            ...
            <div className="container max-w-7xl mx-auto px-4 space-y-8">
                ...
                {filteredList.length === 0 ? (
                    <div className="py-20 text-center bg-card/50 backdrop-blur-xl border border-dashed border-border rounded-3xl">
                        <p className="text-xl text-muted-foreground">No manga found in this category.</p>
                    </div>
                ) : (
                            ...
                            >
                                <Link href={`/manga/${item.mangaId}`}>
                                    <div className="group relative aspect-[2/3] rounded-2xl overflow-hidden bg-muted shadow-lg hover:shadow-primary/20 transition-all border border-border">
                                        {item.mangaImage ? (
                                            <Image src={item.mangaImage} alt={item.mangaTitle} fill className="object-cover transition-transform group-hover:scale-110 duration-500" sizes="(max-width: 768px) 50vw, 20vw" />
                                        ) : (
                                        ...
                                        <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-4">
                                            <p className="text-white font-bold text-sm line-clamp-2">{item.mangaTitle}</p>
                                        </div>
                                    </div>
                                </Link>
                            </motion.div>
